...
const IMPLEMENTATION = { name: "MCP Apps Host - Airbyte Widget", version: "1.0.0" };
...
async function getUiResource(serverInfo: ServerInfo, uri: string): Promise<UiResourceData> {
  ...
  log.info("Resource content._meta:", (content as Record<string, unknown>)._meta);

  // Try both _meta (spec) and meta (Python SDK quirk)
  const contentMeta = (content as Record<string, unknown>)._meta || (content as Record<string, unknown>).meta;
  const csp = (contentMeta as Record<string, unknown>)?.ui ? ((contentMeta as Record<string, unknown>).ui as Record<string, unknown>)?.csp as McpUiResourceCsp : undefined;
  const permissions = (contentMeta as Record<string, unknown>)?.ui ? ((contentMeta as Record<string, unknown>).ui as Record<string, unknown>)?.permissions as McpUiResourcePermissions : undefined;
